USE [SUNRISE10_CDB]
GO
/****** Object:  StoredProcedure [dbo].[pc_RackOut]    Script Date: 06/09/2017 09:15:27 ******/
SET ANSI_NULLS ON
GO
	...
	BEGIN
		...
		BEGIN--顺序确定上线 bug 暂时
			...
			BEGIN
				...
				IF(@timediff > 300 or @timediff is null)--避免重复 切制单
				BEGIN
					IF NOT EXISTS (SELECT TOP 1 1 FROM tZdOnLine WHERE Route_guid=@yzrouteguid AND MODCS_Guid=@modcsguid AND Line_guid=@lineguidYZ AND SubOver=0)	
												BEGIN
					EXEC pm_on_InportZdD			
...
------------------------------------------------------------------------------------------
	...
	IF(@missing=0)	--如果没有遗失（处理衣车故障等）
		...
		-- ----------------------------------------------------------------------
		END
		ELSE IF(@outKind<>4)	--出站类型不是4-返工到质检站
		BEGIN
			--print '正常查找下道工序'
...
--
IF (@stguidDes IS NULL) AND (@outkind<>0) AND (@canOut=1)  AND  (@stkind=8)AND (@isjoin=0)
BEGIN
	SELECT @canOut=0;
	INSERT INTO #tMsg(WorkLine, StationID, Msg, Way)
					SELECT WorkLine, StationID
